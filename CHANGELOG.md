...
- **Commissioni di vendita non dedotte dal corrispettivo Quadro RT.** `quadro_rt.py` calcolava `pnl_eur = proceeds_eur - cost_eur`, ignorando completamente la commissione del broker sul SELL. L'art. 68 co. 6 TUIR include nelle componenti deducibili della plusvalenza "ogni onere inerente alla cessione" — categoria che la prassi (e tutta la modulistica AdE) interpreta come comprensiva delle commissioni di vendita. Effetto: decaf sovrastimava la plusvalenza (o sotto-stimava la minusvalenza) di un ammontare pari alla commissione di vendita convertita al cambio BCE della data di regolamento. **Su Schwab le commissioni sono $0 (sells gratuiti, RSU lapse incluse), quindi il bug colpisce solo i lotti IBKR**: ETF, azioni cash. Esempio reale 0.4.0 vs 0.5.0 su VWRA IE00BK5BQT80 venduto su IBKR il 2025-09-04, 1 azione, proceeds $158.34, cost $158.82, commissione IBKR $-4: la 0.4.0 emetteva minusvalenza EUR di -€0.94, la 0.5.0 emette -€4.35 (matching il `fifoPnlRealized` IBKR di $-4.48 → -€3.82 + arrotondamenti cambi). La commissione di **acquisto** non richiede aggiustamento: IBKR la capitalizza già nel `cost` per-lotto al momento del FIFO match; Schwab non addebita commissione su RSU vest. Vedi `doc/NORMATIVA.md` § "Commissioni di vendita".

### Changed

- **Quadro RT: layout PDF ristrutturato per tracciabilità diretta col Year-End Summary Schwab.** Il PDF mostra ora una colonna per ognuno dei numeri stampati sul broker statement, in modo che ogni cifra EUR sia riconducibile a una riga del PDF sorgente: `VN/sh` (Valore Normale italiano per azione, matching colonna ITA FMV dell'Annual Withholding Statement Schwab), `Broker/sh` (cost basis broker per azione = W-2 USA, matching colonna IRL FMV / colonna "Cost Basis" del Year-End Summary), `Sell/sh` (corrispettivo per azione, matching colonna "Total Proceeds" del Year-End Summary diviso quantità), `Broker cost` (cost basis totale del lotto secondo il broker, in valuta originale col prefisso `$`/`€`/`£`), `Broker P/L` (P/L USD del broker col simbolo di valuta). Aggiunta riga NETTO con somme per valuta: Base/Corrispettivo/Commissione/P/L in EUR, Broker cost/Broker P/L per valuta originale (es. `$244,351.34` per la somma dei lotti USD). La riga NETTO Broker P/L combacia con il "TOTAL REALIZED GAIN OR (LOSS)" del Year-End Summary Schwab al centesimo (a meno di lotti non-Schwab nello stesso anno). Drop colonna `Fx` (sempre vuota — i forex hanno la loro sezione separata) e suffisso `(acquired YYYY-MM-DD)` ridondante nella descrizione (data già nella colonna `Acquisto`).
- **Quadro RT XLS: una colonna per metrica.** Lo sheet "Quadro RT" del file Excel espone una colonna separata per ognuna delle metriche (Costo VN EUR, Costo VN val., Costo VN per azione, Costo broker val., Costo broker per azione, Commissione val., Commissione EUR, P/L broker val., P/L broker EUR) per consentire filtri, pivot e formule sui singoli numeri.
- **PDF: bundled DejaVu Sans per supporto Unicode.** Il PDF usa ora DejaVu Sans (3 TTF, ~2 MB packaged con `decaf-tax`) anziché il built-in Helvetica di fpdf2 (Latin-1 only). I simboli `€`/`£` e i caratteri accentati italiani (`è`, `à`, `ò`) sono resi correttamente sia nelle intestazioni che nei valori delle celle.
- **PDF: intestazioni di tabella ripetute su tutte le pagine.** Quando una tabella (RW, RT, RL) si estende su più pagine, l'intestazione viene rimessa in alto su ogni nuova pagina. Implementato disabilitando l'auto-page-break di fpdf2 per la durata di `data_table` e controllando manualmente lo spazio residuo prima di ogni riga.
- **Schema SQLite `trades`: aggiunta colonna `broker_cost_basis_original`.** Conserva il cost basis originale del broker per ogni SELL trade — prima della sostituzione al Valore Normale per i lotti RSU. Migrazione automatica in place (`ALTER TABLE ADD COLUMN broker_cost_basis_original TEXT NOT NULL DEFAULT '0'`); il successivo `decaf load` esegue un UPDATE-on-default in `_store_trades` per backfillare le righe pre-esistenti senza alterare quelle già popolate. Necessario per esporre la colonna `Broker cost` nel report e mantenere il legame col PDF sorgente.
